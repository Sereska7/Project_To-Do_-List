...
from crud.crud_task import task_create, get_tasks, update_task_with_permission_check
from crud.crud_task import get_tasks_by_user_id, get_task_by_id
...
async def create_task(task_in: TaskCreate, user: UserRead = Depends(get_current_user)):
...
async def delete_task(task_id: int, user: UserRead = Depends(get_current_user)) -> dict:
    task = await delete_task_by_id(task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
